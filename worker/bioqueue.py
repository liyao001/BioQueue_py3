...
def get_steps(protocol_id):
    ...
    for index, step in enumerate(steps):
        ...
        step_list.append({
            'id': index,
            'parameter': html.unescape(str(step.software).rstrip() + " " + str(step.parameter)),
...
def run_prepare(job_id, job, no_new_learn=0):
    ...
    if resource_needed['cpu'] is not None and resource_needed['cpu'] > int(settings['env']['cpu']) * 100:
...
def run_step(job_desc, resources):
    ...
    if settings['cluster']['type'] and not is_force_local:
        ...
        logger.info("Now run %s" % job_desc)
        logger.info("Resource pool: CPU %f, Memory %f, Disk %d" % (CPU_POOL, MEMORY_POOL, DISK_POOL))
        try:
            ...
            logger.info("Now job %s finished." % job_desc)
            JOB_TABLE[job_id]['resume'] = step_order

...
from .forms import SingleJobForm, JobManipulateForm, CreateProtocolForm, ProtocolManipulateForm, CreateStepForm, \
    ...
    CreateVEForm, AddSampleForm
from .models import Queue, ProtocolList, Protocol, Prediction, References, VirtualEnvironment, Experiment, Sample
import os
import re
import base64
        ...
        try:
            ...
            if tool_info["compile_method"] not in get_maintenance_protocols() and tool_info["is_binary"] != "y":
                ...
                if tool_info["is_binary"] != "y":
                    ...
                    m = hashlib.md5(('python ' + compile_tool_command.strip()).encode())
    ...
    if request.method == 'POST':
        ...
        if query_protocol_form.is_valid():
            ...
            if request.user.is_superuser:
                ...
                step_list = Protocol.objects.filter(parent=cd['parent']).filter(user_id=request.user.id).all().order_by(
                    'step_order')
